...
#
# Simulates exactly what the Codex marketplace does when installing a
...
REPO_URL="${REPO_URL:-https://github.com/cockroachdb/codex-plugin.git}"
# No published SHAs yet — pass one as $1, or set REPO_URL to a local file:// URL
...
if [[ $# -ge 1 ]]; then
  ...
  # Default: test current HEAD of REPO_URL
  SHAS=("HEAD")
fi
...
test_sha() {
  ...
  if [[ ${skill_count} -gt 0 && ${broken_count} -eq 0 && ${symlink_count} -eq 0 ]]; then
    echo "  RESULT: PASS -- ${skill_count} skills present as real files"
  elif [[ ${skill_count} -eq 0 ]]; then
    echo "  RESULT: FAIL -- 0 skills discovered (broken symlinks to empty submodule)"
  else
    echo "  RESULT: PARTIAL -- ${skill_count} skills found, ${broken_count} broken, ${symlink_count} as symlinks"
  fi
  echo ""
}
...
echo ""
echo "=================================================================="
echo " CockroachDB Codex Plugin - Marketplace Install Validator"
echo " Repo: ${REPO_URL}"
